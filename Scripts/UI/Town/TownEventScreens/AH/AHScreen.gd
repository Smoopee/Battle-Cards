...
func _input(event):
	if $AHCardManager.visible == true: return
...
func finish_drag():
	...
	if event_found:
		for i in get_tree().get_first_node_in_group("organizer").get_children():
			i.queue_free()
		event_found.effect()
...
func setup_ah(tag):
	if ah_mode == "Cards":
		$AHCardDisplay.visible = true
		$AHCardDisplay.process_mode = Node.PROCESS_MODE_INHERIT
		$AHCardManager.visible = true
		$AHCardManager.process_mode = Node.PROCESS_MODE_INHERIT
		
		$AHCardDisplay.setup(tag)
		
	if ah_mode == "Skills":
		$AHSkillDisplay.visible = true
		$AHSkillDisplay.process_mode = Node.PROCESS_MODE_INHERIT
		$AHSkillManager.visible = true
