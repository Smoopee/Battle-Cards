...
func _ready():
	...
	merchant_inventory_reference = $"../AHSkillDisplay"
...
func finish_drag_skill():
	merchant_inventory_reference.animate_skill_to_position(skill_being_dragged, skill_previous_position)
...
func check_for_upgrade_skill():
	...
	for i in get_tree().get_first_node_in_group("character").get_node("Skills").get_children():
...
func upgrade_skill(skill_being_upgraded):
	if Global.player_gold < skill_being_dragged.skill_stats.buy_price:
		...
		return
	merchant_inventory_reference.remove_skill_from_inventory(skill_being_dragged)
...
func buy_skill():
	...
	get_tree().get_first_node_in_group("character").add_skill(skill_being_dragged.skill_stats)
	merchant_inventory_reference.remove_skill_from_inventory(skill_being_dragged)
	skill_being_dragged.queue_free()
	update_player_gold()
