...
func effect(player_deck, enemy_deck, player, enemy):
	...
	match mode:
		"Battle Stance":
			...
			stats.mode = "Defensive Stance"
			var buff_resource = load('res://Resources/Buffs/defensive_stance.tres')
			target.add_buff(buff_resource, parent)
			target.remove_buff("Battle Stance")
		
		"Defensive Stance":
			print("Entered Battle Stance")
			stats.mode = "Battle Stance"
			var buff_resource = load('res://Resources/Buffs/battle_stance.tres')
			target.add_buff(buff_resource, parent)
			target.remove_buff("Defensive Stance")
		_: 
			print("Entered Defensive Stance")
			stats.mode = "Defensive Stance"
			var buff_resource = load('res://Resources/Buffs/defensive_stance.tres')
			target.add_buff(buff_resource, parent)
			target.change_attack(stats.effect1)
