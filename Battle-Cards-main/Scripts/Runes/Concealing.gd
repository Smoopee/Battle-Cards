...
func rune_effect1():
	...
	for i in get_tree().get_first_node_in_group("enemy deck").get_children():
		if i.is_in_group("card"):
			i.get_node("BaseCard").get_node("CardImage").texture = conceal_card
			i.get_node("BaseCard").get_node("ItemEnchantImage").visible = false
			i.get_node("BaseCard").get_node("UpgradeBorder").visible = false
