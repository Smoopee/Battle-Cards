...
func get_inventory(skill_tag):
	get_inventory_selection(skill_tag)
	create_inventory()
...
func create_skill_selection():
	...
	for i in range(inventory.size()):
		...
		get_tree().get_first_node_in_group("organizer").add_child(skill_scene)
